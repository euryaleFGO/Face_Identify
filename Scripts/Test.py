...
from PyQt5.QtWidgets import QApplication, QWidget, QPushButton, QVBoxLayout, QHBoxLayout, QLabel, QSizePolicy, QMessageBox, QMainWindow, QAction, QFileDialog, QDialog, QSlider, QMessageBox
    ...
    def run(self):
        while True:
            ...
            for face in faces:
                ...
                # 调整绘制矩形和文本的位置以适应缩小后的图像
                cv2.rectangle(frame, (int(x*self.scale_factor), int(y*self.scale_factor)), 
                              (int((x + w)*self.scale_factor), int((y + h)*self.scale_factor)), 
                              (0, 255, 0), 1)
                cv2.putText(frame, identity_label, 
                            (int(x*self.scale_factor), int(y*self.scale_factor - 10)), 
                            cv2.FONT_HERSHEY_SIMPLEX, 0.7*self.scale_factor, (0, 255, 0), 1)
...
class FaceRecognitionWindow(QMainWindow):
    def __init__(self):
        self.is_fullscreen = False  # 添加一个属性来追踪是否是全屏
    ...
    def initUI(self):
        ...
        self.thread = self.video_thread  # 使用同一个线程
    ...
    # 创建“选项”下的子菜单或动作，并连接到适当的槽
        ...
        # 全屏和退出全屏的子选项
        self.toggle_fullscreen_action = QAction('全屏', self)
        options_menu.addAction(self.toggle_fullscreen_action)
        self.toggle_fullscreen_action.triggered.connect(self.toggle_fullscreen)
        
        # 调整窗口大小的子选项
        self.resize_window_action = QAction('调整窗口大小', self)
        options_menu.addAction(self.resize_window_action)
        self.resize_window_action.triggered.connect(self.resize_window)
            # 创建“帮助”菜单
...
# 连接动作到槽函数（假设您已经有了实现这些功能的方法）
    ...
    def toggle_fullscreen(self):
        if self.isFullScreen():
            self.normalGeometry = self.normalGeometry or self.geometry()
            self.showNormal()
            self.setGeometry(self.normalGeometry)
            self.toggle_fullscreen_action.setText('全屏')
            self.set_video_scale(False)  # 通知视频线程缩小视频帧
        else:
            self.normalGeometry = self.geometry()
            self.showFullScreen()
            self.toggle_fullscreen_action.setText('退出全屏')
            self.set_video_scale(True)  # 通知视频线程放大视频帧至原始大小

        self.label.adjustSize()

    def set_video_scale(self, is_fullscreen):
        """设置视频缩放比例"""
        self.is_fullscreen = is_fullscreen
        scale_factor = 0.5 if not is_fullscreen else 1.0
        self.video_thread.set_scale_factor(scale_factor)

    def resize_window(self):
        self.resize_dialog = QDialog(self)
        self.resize_dialog.setWindowTitle('调整窗口大小')
        layout = QVBoxLayout()
        ...
        self.height_slider.setMinimum(300)
        self.height_slider.setMaximum(1080)
        ...
        save_button.clicked.connect(self.save_window_size)

        exit_button = QPushButton('退出')
        exit_button.clicked.connect(self.resize_dialog.reject)
        ...
        self.resize_dialog.setLayout(layout)
        self.resize_dialog.exec_()

    def save_window_size(self):
        width = self.width_slider.value()
        height = self.height_slider.value()
        self.resize_dialog.reject()
    ...
    def add_new_face(self):
        self.video_thread.cap.grab()
        ret, frame = self.video_thread.cap.retrieve()
        if not ret:
    ...
    def show_options(self):
        ...
        QMessageBox.information(self, '选项', '设置功能尚未实现')
    def show_help(self):
